...
async function get_airport_data(icao_code) {
    let path = "https://api.core.openaip.net/api/airports?search=" + icao_code;
    let headers = {
        "x-openaip-api-key": API_KEY,
        "accept": "application/json"
    };
    try {
        ...
        if (response.ok) {
            ...
            console.error(`Failed to download airport data: ${response.status}, ${response.statusText}`);
        }
    } catch (error) {
